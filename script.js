...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Identity Persistence (Session simulation)
    ...
    const showPage = (pageId) => {
        ...
        document.querySelectorAll('.nav-item').forEach(link => {
            ...
            // We add these base classes, but they might conflict if we don't manage specific text colors well
            // Simpler: Just rely on active-nav for the active state styling
    ...
    const updateAuthUI = () => {
        ...
        container.querySelectorAll('.nav-link').forEach(l => l.addEventListener('click', (e) => {
            e.preventDefault();
            // If it's a link with an icon, target might be the icon, so we use currentTarget or closest
    ...
    // Form Submission Events (Login/Register)
    ...
    document.getElementById('room-booking-form').addEventListener('submit', (e) => {
        ...
        if(bookings.find(b => b.roomId == roomId && b.slot == slot)) {
            ...
            // Links bookings specifically to the User ID (ownership)
            bookings.push({
                id: Date.now(),
                userId: currentUser.id,// <--- Virtual Identity Link
                roomId,
    ...
    const renderAccount = () => {
        ...
        // Checks the identity's username property
